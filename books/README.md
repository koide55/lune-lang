...
```sh
BOOK=lune-book-en books/tools/build_pdf.sh out.pdf
```

**英語版で1つ分かったこと**: ページ番号を入れると目次自体の行数が変わり、その分だけ
本文がずれることがある（章題が長いため、実際に1ページずれた）。そこで `build_pdf.sh` は
**ずれなくなるまで組み直す**ようにした。日本語版は2パス、英語版は3パスで落ち着く。
落ち着かなければ警告を出す（目次が1行増減するたびに本文が1ページ動く「境界」に
当たっている状態なので、人が見るべき）。索引語は `book_pages.py` の `CONCEPTS_EN`。
